...
$ kubectl config get-contexts # check your current context and namespace
```
...
5. 1. Create deployment `backend-depl` using nginx image. Use `db-config` configmap. 
5. 2. Check the current `DB_USERNAME` env. Change the `DB_USERNAME` value to `sqlite`.
5. 3. Restart your pod and make sure all container from `backend-depl` have the new value of `DB_USERNAME`
